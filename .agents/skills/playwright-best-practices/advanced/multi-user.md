...
test("handles concurrent edits", async ({ browser }) => {
    ...
    await Promise.all([page1.getByRole("button", { name: "Edit" }).click(), page2.getByRole("button", { name: "Edit" }).click()]);
    ...
    await Promise.all([page1.getByRole("button", { name: "Save" }).click(), page2.getByRole("button", { name: "Save" }).click()]);
