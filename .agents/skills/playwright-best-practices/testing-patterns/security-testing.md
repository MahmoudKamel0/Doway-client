...
test("SQL injection is prevented", async ({ page }) => {
    const sqlPayloads = ["'; DROP TABLE users; --", "1' OR '1'='1", "1; DELETE FROM orders", "' UNION SELECT * FROM users --"];
...
> **For comprehensive console monitoring** (fixtures, allowed patterns, fail on errors), see
> [console-errors.md](../debugging/console-errors.md).
