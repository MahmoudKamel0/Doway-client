...
const responsePromise = page.waitForResponse((r) => r.url().includes("/api/data") && r.request().method() === "GET" && r.ok());
await page.click("#load-data");
await responsePromise;
await expect(page.locator(".data-row")).toHaveCount(10);
```

> **For comprehensive waiting strategies** (navigation, element state, network, polling with `toPass()`), see
> [assertions-waiting.md](assertions-waiting.md#waiting-strategies).
