...
export const test = base.extend<{}, { collectCoverage: void }>({
    collectCoverage: [
        async ({ browser }, use) => {
            ...
            fs.writeFileSync(path.join(coverageDir, `coverage-${randomUUID()}.json`), JSON.stringify([...jsCoverage, ...cssCoverage]));
...
for (const file of changedFiles) {
    ...
    if (entry) {
        const percent = (entry.ranges.reduce((s: number, r: any) => s + r.end - r.start, 0) / (entry.text?.length || 1)) * 100;
