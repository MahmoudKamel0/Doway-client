...
> **When to use**: Testing Angular applications with reactive forms, Angular Material components, Router navigation, lazy-loaded modules,
> signals, observables, and Zone.js change detection. **Prerequisites**: [core/configuration.md](../core/configuration.md),
...
- **Long-running async**: `setInterval` or long-running observables keep Angular "not stable." This does not affect Playwright (unlike
  Protractor).
...
Angular Material and CDK render overlays (dialogs, menus, selects) in a special container outside the component tree. Playwright sees these
as regular DOM elements:
