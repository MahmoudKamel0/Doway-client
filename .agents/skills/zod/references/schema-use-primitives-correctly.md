...
impact: CRITICAL
impactDescription:
    Incorrect primitive selection causes validation to pass on wrong types; using z.any() or z.unknown() loses all type safety
tags: schema, primitives, types, basics
---

## Use Primitive Schemas Correctly

Zod provides specific schemas for each primitive type. Using the wrong schema (e.g., `z.string()` when you need `z.number()`) or falling
back to `z.any()` defeats the purpose of validation entirely, allowing corrupt data through.
